...
if __name__ == "__main__":
    ...
    max_days = (
        int(sys.argv[3]) if len(sys.argv) > 3 and sys.argv[2] == "--max-days" else None
    )
    max_downloads = (
        int(sys.argv[3]) if len(sys.argv) > 3 and sys.argv[2] == "--max-down" else None
    )
    encrypt_password = (
        sys.argv[3] if len(sys.argv) > 3 and sys.argv[2] == "--password" else None
    )
